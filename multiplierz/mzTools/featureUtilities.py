...
def save_feature_csv(features, outputfile, datafile, overwrite = False):
    """
    Saves a CSV feature database. Result file will have the
    extension '.features' .
    """
    ...
    for index, feature in enumerate(features):
        row = {}
        row['mz'] = feature.mz
        row['startscan'], row['endscan'] = feature.scanrange
        row['startscantimemins'] = data.time_for_scan(row['startscan'])
        row['endscantimemins'] = data.time_for_scan(row['endscan'])
        row['charge'] = feature.charge
        row['topSignal'] = feature.topSignal()
        row['totalIntensity'] = feature.totalIntensity()
        row['c12Intensity'] = feature.c12Intensity()
